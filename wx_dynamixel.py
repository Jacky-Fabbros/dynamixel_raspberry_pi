...
class dynamixel():
    def __init__(self, port=Port()):
        ...
        self.kins = ({'MX_READ_DATA':2, 'MX_WRITE_DATA':3})
    ...
    def set_reg(self, index, ins, params=[]):
        if self.s_port.isOpen():#should be able to remove the try and replace with if serial is available
            ...
            print "dynamixel set_mx_reg error no port open"
            return False
        return True
    ...
    def set_mx_reg(self, ID, reg, values=list(), ins = WRITE_DATA):
...
if __name__ == '__main__':
    ...
    mx28.set_mx_reg(ID, SPEED_REG, ([(speed%256),(speed>>8)]))
    #test that the speed is set corectly
    return_speed =  mx28.get_reg(ID, ins=READDATA, regstart=SPEED_REG, rlength=1)
    if return_speed:
        ...
        for pos, delay in positions_delay:
            mx28.set_mx_reg(ID, POS_REG, ([(pos%256),(pos>>8)]))
            time.sleep(delay)
    else:
        print "error setting and getting mx data"
    print "test complete"